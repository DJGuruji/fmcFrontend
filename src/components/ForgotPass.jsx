...
const ForgotPass = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success("A password reset link has been sent to your email. Reset it before the link expires.");
    } catch (error) {
      ...
      <div className="max-w-md w-full space-y-8 dark:bg-slate-800 p-5">
        ...
        <form className="mt-8 space-y-6 shadow-lg" onSubmit={handleSubmit}>
          <div className="rounded-md shadow-sm -space-y-px">
            <label htmlFor="email" className="sr-only dark:text-white">Email address</label>
          ...
          <p className="ml-7 lg:ml-9 xl:ml-9 mt-5 dark:text-white">
            <a
              ...
              className="ml-1 text-blue-700 hover:underline hover:text-green-700 cursor-pointer"
            >
              Back to login?
            </a>
          </p>
